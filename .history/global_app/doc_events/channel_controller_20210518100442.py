...
def get_sales_order_count(po_no):
    ...
    count_no_of_open_so = frappe.db.get_all("Sales Order", filters={"po_no": po_no,"status": "To Deliver and Bill"}, fields=['*'])
...
def update_packing_slip(cost, wt, name):
    ...
    x = frappe.db.sql("""SELECT shipment_cost,weight FROM `tabPacking Slip packages shadow` WHERE name = %s""", name , as_dict=1)
    frappe.db.commit()
    return x
...
def update_all(order_id):
    ...
    try:
        so_list = frappe.get_all("Sales Order", filters={"po_no": order_id}, fields=['*'])
        cc_list = frappe.get_all("Channel Controller", filters={"order_id": order_id}, fields=['*'])

        so_doc = frappe.get_doc("Sales Order", so_list[0].name)

        ps_list = frappe.db.get_all("Packing Slip", filters={"purchase_order": order_id}, fields=['*'])
        ...
        remote_update_sales_order(tracking_number, shipping_cost, order_id, shipment_weight)
        remote_update_channel_controller(order_id, shipping_carrier, shipping_service, tracking_number, date)
...
def remote_update_channel_controller(order_id,carrier_code,shipping_service,tracking_id,date):
    ...
    remote_cc = conn.get_list("Channel Controller", filters={"order_id": order_id})
...
def update_remote_tracking_number():
    cc_list = frappe.get_all("Channel Controller", filters=[ ['Controller Tracking Table','tracking_id', '!=', '']] , fields=['name','order_id'])
    ...
    for cc_order_id in cc_list:
        ...
        try:
            ...
            so_list = frappe.get_list("Sales Order", filters={"po_no":cc_order_id.order_id})
            ...
            for tracking_info in tracking_info_list:
                ...
                try:
                    ...
                    remote_update_sales_order(tracking_id, shipment_cost, cc_order_id.order_id, shipment_weight)
                    remote_update_channel_controller(cc_order_id.order_id, tracking_info_list[0].carrier, tracking_info_list[0].method, tracking_info_list[0].tracking_id,  tracking_info_list[0].shipped_date)
