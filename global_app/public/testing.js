...
// cur_frm.cscript.packages_information_on_form_rendered = function (doc, cdt, cdn) {
//     var d = locals[cdt][cdn]
//     var df5 = frappe.meta.get_docfield("Packing Slip packages shadow", "shipping_label", cur_frm.doc.name);
//     df5.hidden = !(d.shipment_tracking_number === "")
    ...
    add_to_box: function (frm) {
        ...
        frm.doc.pick_items.forEach(function (item) {
            ...
            if (item.__checked) {
                ...
                //weight_str+=parseFloat(item.net_weight);
...
frappe.ui.form.on("Packing Slip packages shadow", {
    ...
    packages_information_add: function (frm) {
        //alert('added');
    },
    packages_information_remove: function (frm) {
        //alert('removed...........');
    },
...
function handle_batch_request_response(frm, data, dl) {
    ...
    // if(data.Fault.detail.Errors.ErrorDetail.PrimaryErrorCode){
    // 	frappe.msgprint(data.Fault.detail.Errors.ErrorDetail.PrimaryErrorCode.Description,"err");
    // }
...
function update_sales_order(frm, data, dl, box) {
    ...
    var inh_ins = 0.0;
    if (frm.doc.inhouse_insurance) {
        inh_ins = frm.doc.inhouse_insurance;
    }
...
function ne_packing_logic(frm) {
    ...
    frappe.model.clear_table(frm.doc, "pick_items");
...
function pack_ups_items(frm) {
    ...
    if (list_item_code.length > 0) {
        frm.call({
            ...
            callback: function(response) {
                var data = response.message
                console.log("Response IN UPS ITEMS PACCKE")
                console.log(data)
                if (data) {
 var items_qty = "";
                    var item_str = ''
                    ...
                    // frappe.model.set_value(pck_inf.doctype, pck_inf.name, "weight", data.weight);
                    frappe.model.set_value(pck_inf.doctype, pck_inf.name, "items", item_str);
                    frappe.model.set_value(pck_inf.doctype, pck_inf.name, "price", price);
...
function print_shipping_cost(frm, cdt, cdn){
        if (frm.doc.name.indexOf("New Packing Slip") > -1) {
            frappe.msgprint("Please Save the document first")
            return false;
        }
