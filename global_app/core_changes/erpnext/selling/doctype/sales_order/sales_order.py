...
class SalesOrder(SellingController):
	...
	def validate_po_uniqness(self):
		if self.po_no:
			so= frappe.db.get_all("Sales Order",filters={'po_no':self.po_no,'name':('!=',self.name),'status':('!=','Cancelled')},fields=['name'])
			if len(so) > 0:
				frappe.throw(_("There is already an order with same purchase order number"))
				return False
...
def get_stock_balance(item_code):
		doc = frappe.db.get_all("Bin",filters={"item_code":item_code},fields=["actual_qty"])
		try:
			return doc[0].actual_qty
		except:
			return "Non stock"
...
@frappe.whitelist()
def check_for_payment_PBS(po_no,customer):
	inv = frappe.get_all("Sales Invoice",filters={"po_no":po_no,"customer":customer,"status":"Paid"},fields=["*"])
	if len(inv)>0:
		return True
	else:
		return False