...
# return base64.encodestring(md.digest().encode('utf-8')).strip('\n')
...
class DictWrapper(object):
    ...
    # frappe.msgprint(self.original)

    @property
    def parsed(self):
        # frappe.msgprint(self._rootkey)
        # GetFeedSubmissionResultResult
        if self._rootkey != 'GetFeedSubmissionResultResult':
            # if (self._rootkey!='GetOrderResult'):
...
class MWS(object):
    ...
    def __init__(self, access_key, secret_key, authtoken, account_id, region='US', domain='', uri="", version=""):
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        params = {
            ...
            'MWSAuthToken': self.authtoken
        }
        params.update(extra_data)
        request_description = '&'.join(
            ['%s=%s' % (k, urllib.parse.quote(str(params[k]).encode('utf-8'), safe='-_.~')) for k in sorted(params)])
...
class Feeds(MWS):
    ...
    VERSION = "2009-01-01"
    NS = '{https://mws.amazonservices.com/Feeds/2009-01-01}'
    ...
    # return self.make_request(data, method="POST", body=feed,extra_headers={'Content-MD5': md, 'Content-Type': content_type})
    ...
    def get_feed_submission_result(self, feedid):
        data = dict(Action='GetFeedSubmissionResult', FeedSubmissionId='50127018062')
        return self.make_request(data)
...
class OutboundShipments(MWS):
    ...
    def List_All_Fulfillment_Orders(self, SellerFulfillmentOrderId=None):
        data = dict(Action='GetServiceStatus')
        return self.make_request(data)

    def Create_Fulfillment_Orders(self, SellerFulfillmentOrderId=None,
                                  FulfillmentAction=None, DisplayableOrderId=None,
                                  DisplayableOrderDateTime=None, DisplayableOrderComment=None,
                                  ShippingSpeedCategory=None,
                                  Name=(), Line1=(), Line2=(), Line3=(), city=(), StateOrProvinceCode=(),
                                  CountryCode=(),
                                  PostalCode=(), item_sku=(), item_qty=()):
        data = dict(Action='CreateFulfillmentOrder', SellerFulfillmentOrderId=SellerFulfillmentOrderId,
                    FulfillmentAction=FulfillmentAction, DisplayableOrderId=DisplayableOrderId,
                    DisplayableOrderDateTime=DisplayableOrderDateTime, DisplayableOrderComment=DisplayableOrderComment,
                    ShippingSpeedCategory=ShippingSpeedCategory
                    )
        tracking = [SellerFulfillmentOrderId]
        data.update(self.enumerate_param('DestinationAddress.Name', Name))
        data.update(self.enumerate_param('DestinationAddress.Line1', Line1))
        data.update(self.enumerate_param('DestinationAddress.Line2', Line2))
        data.update(self.enumerate_param('DestinationAddress.Line3', Line3))
        data.update(self.enumerate_param('DestinationAddress.city', city))
        data.update(self.enumerate_param('DestinationAddress.StateOrProvinceCode', StateOrProvinceCode))
        data.update(self.enumerate_param('DestinationAddress.CountryCode', CountryCode))
        data.update(self.enumerate_param('DestinationAddress.PostalCode', PostalCode))
        data.update(self.enumerate_param('Items.member.SellerSKU', item_sku))
        data.update(self.enumerate_param('Items.member.Quantity', item_qty))
        data.update(self.enumerate_param('Items.member.SellerFulfillmentOrderItemId', tracking))
        return data
