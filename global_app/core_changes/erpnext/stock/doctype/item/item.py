...
@frappe.whitelist()
def update_images():
	items=frappe.get_all("Item",filters={},fields=["*"])
	count=0
	for item in items:
		if item.item_group=="Stock":
			path="/files/Stock/"+item.sub_group+"/"+item.item_code+".jpg"
			if "GD" in item.item_code:
				path="/files/Stock/GD/"+item.item_code+".jpg"
			frappe.db.sql("""update tabItem set image=%s, thumbnail=%s where item_code=%s""", (path,path, item.item_code))
			frappe.db.commit()
		elif item.item_group=="Custom":
			path="/files/Custom/"+item.sub_group+"/"+item.item_code+".jpg"
			frappe.db.sql("""update tabItem set image=%s, thumbnail=%s where item_code=%s""", (path,path, item.item_code))
			frappe.db.commit()
...
def update_valuation():
	...
	for item in items:
		if item.item_group == "Stock":
			frappe.db.sql("""update tabItem set valuation_rate=%s where item_code=%s""", (1.00, item.item_code))
			frappe.db.commit()
		count+=1
	print(count)
...
def update_item_prices():
	item_price= frappe.get_all("Item Price",filters={},fields=["*"])
	items=frappe.get_all("Item",filters={'color_1':"Digital"},fields=["item_code"])
	item_list=[]
	for itm in items:
		item_list.append(itm.item_code.strip())
	print(len(item_price))
	print(len(item_list))
	count=0
	for item_p in item_price:
		#print(item_p.item_code)
		# flg=False
		# for itm in item_list:
		# 		# frappe.db.sql("""update tabItem set valuation_rate=%s where item_code=%s""", (1.00, item.item_code))
		# 		# frappe.db.commit()
		# 		if item_p.item_code ==itm:
		# 			count+=1
		# 			print(item_p.item_code)
		# 			flg=True
		# if not flg:
		color=frappe.get_doc("Item",item_p.item_code)
		frappe.db.sql(""" update `tabItem Price` set color_1=%s where item_code=%s""", ( color.color_1,item_p.item_code))
		frappe.db.commit()
	print(count)
@frappe.whitelist()
def check_for_missing_items():
	...
	missing_items=[]
	for itm in items:
		dest_item.append(itm.item_code)
	for item in item_list:
		if item in dest_item:
			pass
		else:
...
def edit_item():
	items= frappe.get_all("Item",filters={},fields=["*"])
	for itm in items:
		if itm.item_group =="Stock" and itm.sub_group != None:
			frappe.db.sql(""" update `tabItem` set item_group=%s where name=%s""", ( itm.sub_group,itm.name))
			frappe.db.commit()

	#print(missing_items)erpnext.stock.doctype.item.item.edit_item