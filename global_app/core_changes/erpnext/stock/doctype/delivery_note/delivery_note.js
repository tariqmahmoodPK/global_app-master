	...
	refresh: function(frm) {
		...
		frappe.call({
					...
					callback(response) {
						...
						if(response.message.do_not_use_billing__address ==1){
							console.log(frm.doc.address_display);
							// if(frm.doc.customer_address != ""){
							// 	//alert('ok');
							// 	frappe.model.set_value(frm.doc.doctype, frm.doc.name, "customer_address", "");
							// 	frappe.model.set_value(frm.doc.doctype, frm.doc.name, "address_display", "");
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					frappe.call(
						{
							method: "erpnext.stock.doctype.delivery_note.delivery_note.check_for_packing_slip",
							args: {
							...
							callback(ps) {
									if(ps.message=="True"){
                                        frappe.set_route('List', 'Packing Slip', {delivery_note: cur_frm.doc.name});
									}
									else{
										frappe.model.open_mapped_doc({
