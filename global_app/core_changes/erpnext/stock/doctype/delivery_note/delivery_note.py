...
class DeliveryNote(SellingController):
	...
	def on_trash(self):
				res = frappe.db.sql("""SELECT name FROM `tabPacking Slip` WHERE delivery_note = %s""", self.name)

				if res:
					frappe.throw(_("Please cancel the label and delete packing slip first"))
...
@frappe.whitelist()
def check_for_packing_slip(name):
	doclist = frappe.get_all("Packing Slip",filters={'delivery_note':name},fields=['*'])
	if len(doclist) != 0:
		return "True"
	return False
...
def test_post_request():
	url="https://eyconsrv.com/api/method/erpnext.erpnext_integrations.connectors.MP_connector.testing_sync_outside"
...
@frappe.whitelist()
def validate_shipment(dn):
		ps =  frappe.get_all("Packing Slip", filters={'delivery_note':dn},fields=['*'])
		if len(ps) > 0:
			for pck in ps:
				if pck.shipment != "done":
					return False
		else:
			return False
		return True
...
@frappe.whitelist()
def manual_submit(dn):
		frappe.get_doc("Delivery Note", dn).submit()
		return True
