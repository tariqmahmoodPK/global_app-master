...
def make_payment_request(args):
	value_doc= json.loads(args)
 
	inv_ref=value_doc['references']
	inv_len=len(inv_ref)
	if(len(inv_ref)==1):
		#for inv in inv_ref:
			#description="Payment Request for "+inv['reference_name'] 
		description="Payment Request for "+value_doc['references'][0]['po_no']	
		po_no= value_doc['references'][0]['po_no']
	else:
		description="Multiple INVOICES "+value_doc['name']
		po_no=value_doc['name']
	
	so_fields=frappe.db.get_all("AuthorizeNet Request",filters={"reference_doctype":"Payment Entry","reference_docname":value_doc['name']},fields=["name"])
	if len(so_fields)==0: 
		new_apr= frappe.new_doc("AuthorizeNet Request")
		new_apr.amount=value_doc['paid_amount']
		new_apr.title=value_doc['party']  
		new_apr.reference_doctype=value_doc['doctype']
		new_apr.reference_docname=value_doc['name']
		if "contact_email" not in value_doc:
			contact_record = frappe.get_doc('Contact', value_doc['contact_person'])
			new_apr.payer_email=contact_record.email_id
		else:
			new_apr.payer_email=value_doc['contact_email']

		new_apr.payer_name=value_doc['party']
		new_apr.contact=value_doc['contact_person']
		new_apr.currency='USD'
		new_apr.description=description
		new_apr.po_no=po_no
		new_apr.order_id=value_doc['name']
		new_apr.status="Issued"  
		new_apr.insert()   
		frappe.db.commit() 
		return "success"
	else:
		new_apr= frappe.get_doc("AuthorizeNet Request", value_doc['name'])
		contact_email_value = ""
		if "contact_email" not in value_doc:
			contact_record = frappe.get_doc('Contact', value_doc['contact_person'])
			contact_email_value = contact_record.email_id
		else:
			contact_email_value = value_doc['contact_email']
		new_apr.update({
				"amount":value_doc['paid_amount'],
				"title":value_doc['party'],
				"reference_doctype":value_doc['doctype'],
				"reference_docname":value_doc['name'],
				"payer_email":contact_email_value,
				"payer_name":value_doc['party'],
				"contact":value_doc['contact_person'],
				"currency":'USD',
				"description":description
		})
		new_apr.save(ignore_permissions=True) 
		frappe.db.commit()
		return "success"
	...
	"""return controller.get_payment_url(**{
		...
		"reference_doctype": value['doctype'],
		"reference_docname":value['docname'],
		"payer_email": self.email_to or frappe.session.user,
		"payer_name": frappe.safe_encode(data.customer_name),
		"order_id": self.name,
		"currency": self.currency,
		"po_no":data.po_no.encode("utf-8")
	})"""  
...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		if(self.mode_of_payment=='Credit Card'):
			auth=frappe.get_doc('AuthorizeNet Request', self.name)
			if(auth.status!='Captured'):    
				frappe.throw(_("Can not submit. Credit card payment is not made as yet"))
		...
		self.send_email()
		self.update_invoices_info()


	def update_invoices_info(self):
		try:
			payment_mode = self.mode_of_payment #cheque
			if payment_mode == "Credit Card":
				card_type = self.card_type if self.card_type else ""
				name_on_card = self.name_on_card if self.name_on_card else ""
				authorizenet_id = self.authorizenet_id if self.authorizenet_id else ""
				credit_card = self.credit_card if self.credit_card else ""
				for d in self.get("references"):
					if d.reference_doctype == "Sales Invoice":
						frappe.db.sql('''update `tabSales Invoice` set payment_mode=%s, card_type=%s,name_on_card=%s,
						authorizenet_id = %s, credit_card=%s where name=%s''',(payment_mode,card_type,name_on_card,
						authorizenet_id,credit_card,d.reference_name))
				frappe.db.commit()
			elif payment_mode == "Cheque":
				check_no = self.reference_no
				for d in self.get("references"):
					if d.reference_doctype == "Sales Invoice":
						frappe.db.sql('''update `tabSales Invoice` set payment_mode=%s,check_no=%s where name=%s''',
						(payment_mode,check_no,d.reference_name))
				frappe.db.commit()
		except:
			frappe.log_error(frappe.get_traceback(), 'Payment Entry '+self.name)
	...
	def send_email(self):
				try:
					...
					if self.contact_person:
						...
						cust_cnt = frappe.get_doc("Customer",self.party)
						contact = frappe.get_doc("Contact",cust_cnt.customer_primary_contact)
					recepient=contact.billing_email
					if recepient:
						email_args = {
							"recipients": recepient,
							"sender": None,
							"subject": "Payment Received ${0}/-".format(self.paid_amount),
							"message": """ Dear <b>{0}</b> ,
											<br>
											Thank you for your payment please review the charge details in attached file.
											<br>
							...
							""".format(self.party),
							"now": True,
							"attachments": [frappe.attach_print("Payment Entry", self.name,
								file_name=self.name, print_format="Payment Entry Print")]}
						enqueue(method=frappe.sendmail, queue='short', timeout=300, is_async=True, **email_args)
						frappe.msgprint("Transaction email sent successfuly ")
					else:
	...
	def set_missing_values(self):
		...
		try:
			if not self.contact_person:
				c_customer = frappe.get_doc("Customer",self.party)
				c_contact = frappe.get_doc("Contact", c_customer.customer_primary_contact)
				self.contact_person = c_customer.customer_primary_contact
				self.contact_email = c_contact.billing_email
		except:
			frappe.msgprint("Please check if customer primary contact and billing email exists","Missing Primary Contact")
...
def get_negative_outstanding_invoices(party_type, party, party_account,
	...
	return frappe.db.sql("""
		select
			"{voucher_type}" as voucher_type, name as voucher_no,po_no,
			if({rounded_total_field}, {rounded_total_field}, {grand_total_field}) as invoice_amount,
...
@frappe.whitelist()
def update_invoice_from_pe():
	PE_rec = frappe.db.get_all("Payment Entry",filters={"docstatus":1},fields=["*"])
	for item in PE_rec: #erpnext.accounts.doctype.payment_entry.payment_entry.update_invoice_from_pe
		PE = frappe.get_doc("Payment Entry",item.name)
		payment_mode = PE.mode_of_payment #cheque
		# if payment_mode == "Credit Card":
		# 	card_type = PE.card_type if PE.card_type else ""
		# 	name_on_card = PE.name_on_card if PE.name_on_card else ""
		# 	authorizenet_id = PE.authorizenet_id if PE.authorizenet_id else ""
		# 	credit_card = PE.credit_card if PE.credit_card else ""
		...
		# 			authorizenet_id,credit_card,d.reference_name))
		# 	frappe.db.commit()
		# elif payment_mode == "Cheque": erpnext.accounts.doctype.payment_entry.payment_entry.update_invoice_from_pe
		# 	check_no = PE.reference_no
		# 	for d in PE.get("references"):
		# 		if d.reference_doctype == "Sales Invoice":
		# 			frappe.db.sql('''update `tabSales Invoice` set payment_mode=%s,check_no=%s where name=%s''',
		# 			(payment_mode,check_no,d.reference_name))
		# 	frappe.db.commit()
		for inv in PE.get("references"):
			invoice = frappe.get_doc("Sales Invoice",inv.reference_name)
