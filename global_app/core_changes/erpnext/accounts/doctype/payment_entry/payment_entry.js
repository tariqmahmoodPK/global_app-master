...
frappe.ui.form.on('Payment Entry', {
	...
	after_save:function(frm){
		...
		if(frm.doc.mode_of_payment=='Credit Card'){
			frappe.call({
				//D:\eycon\cobb_stagging\apps\erpnext\erpnext\accounts\doctype\payment_entry\payment_entry.py
				method: "erpnext.accounts.doctype.payment_entry.payment_entry.make_payment_request",
				args: {
					'args':frm.doc
				},
	...
	validate:function(frm){
		//this function is triggered on save
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==0 && frm.doc.mode_of_payment=='Credit Card' && frm.doc.is_authorize=='yes'){ 
			frm.add_custom_button("Refresh", function(){
				location.reload(true) 
			}),  	
			frappe.db.get_doc('AuthorizeNet Request', frm.doc.name)
			.then(doc => {
				frappe.db.get_doc('Customer', frm.doc.party_name)
				.then(doc_c => {
					//console.log(doc_c)     
					if(doc.status=='Error' && doc_c.account_on_hold=='0')	{       
						frm.add_custom_button("Put On HOLD", function(){ 							
							//r.message.set_value('account_on_hold',1);
							frappe.call({
								method: "erpnext.accounts.doctype.payment_entry.payment_entry.put_on_hold",
								args: {
									'args':frm.doc
								}, 
								callback:function(re){
					...
					} else if(doc.status=='Error' && doc_c.account_on_hold=='1')	{
						frm.add_custom_button("Remove On HOLD", function(){ 							
							//r.message.set_value('account_on_hold',1);
							frappe.call({
								method: "erpnext.accounts.doctype.payment_entry.payment_entry.remove_on_hold",
								args: {
									'args':frm.doc
								},
								callback:function(re){
		...
		if(frm.doc.docstatus==0 && frm.doc.mode_of_payment=='Credit Card'){
			frm.add_custom_button("Create AuthorizeNet Request", function(){ 
				if(frm.doc.is_authorize=='yes'){
					frappe.db.get_doc('AuthorizeNet Request', frm.doc.name)
					.then(doc => {
						//console.log(doc_c)     
						if(doc.status!='Captured'){
							frappe.call({
								...
								callback:function(re){
									...
									window.open(url1);
									cur_frm.refresh_field("is_authorize");
								}
							})							
						}else{
							frappe.msgprint("Paymnet already successfully paid");
						}
					})
				}else{
					frappe.call({
			...
			callback: function(r, rt) {
				if(r.message) {
					...
					$.each(r.message, function(i, d) {
						if(d.voucher_type == 'Sales Invoice' && count <=150){
						count=count+1;
						console.log(count)
