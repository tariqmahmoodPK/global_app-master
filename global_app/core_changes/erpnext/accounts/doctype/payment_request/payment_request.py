...
class PaymentRequest(Document):
	...
	def on_submit(self):
		...
		if send_mail:
			...
			"""doc=frappe.get_doc("Customer",ref_doc.customer)
			transaction={
				'amount': ref_doc.total,
			...
			order= {
					'invoice_number': self.reference_name,
					'description': 'Just another invoice',
					'order_number': self.amended_from,
			}
			#if (hasattr(ref_doc, "shipment_amount") and hasattr(ref_doc, "shipment_name") and  hasattr(ref_doc, "shipment_description")):				
			if (ref_doc.shipment_amount and ref_doc.shipment_name and ref_doc.shipment_description):				
					shipping_and_handling= {
							'amount': ref_doc.shipment_amount,
							'name': ref_doc.shipment_name,
							'description':ref_doc.shipment_description,
					} 
			...
			if (hasattr(ref_doc, "total_taxes_and_charges")):
					tax={ 
						'amount': ref_doc.total_taxes_and_charges,
						'name': 'Double Taxation Tax',
						'description': 'Another tax for paying double tax',
					}
			if(bool(line_items)):
					transaction.update({"line_items":lst_line_item})
			if(bool(tax)):
					transaction.update({"tax":tax})
			if(bool(order)):
					transaction.update({"order":order})
			if(bool(shipping_and_handling)):
					transaction.update({"shipping_and_handling":shipping_and_handling})
			if(bool(extra_options)):
					transaction.update({"extra_options":extra_options})
			...
			if(doc.customer_id!="0" and doc.payment_id!="0" and doc.shipping_id!="0"):
				try:		 		
					...
					result = authorize.Transaction.sale(transaction) 
					if(result.transaction_response.trans_id):
							self.db_set('status', 'Paid')  
							self.db_set('transaction_id',result.transaction_response.trans_id)
							ref_doc.db_set('status', 'Paid') 
							ref_doc.db_set('outstanding_amount',0) 
							#frappe.db.commit() 
							authnet_log= frappe.get_list("AuthorizeNet Request", fields=["name"], filters={"reference_docname": self.name}, as_list=1)
							authnet_log_user_name = authnet_log[0][0]
							request_authnet_log = frappe.get_doc("AuthorizeNet Request",authnet_log_user_name)
							request_authnet_log.transaction_id=result.transaction_response.trans_id
							request_authnet_log.status='Captured'
							request_authnet_log.save()
							frappe.db.commit()
					...
					authnet_log_user_name = authnet_log[0][0]
					request_authnet_log = frappe.get_doc("AuthorizeNet Request",authnet_log_user_name)
					request_authnet_log.status='Error'
					request_authnet_log.save()
					frappe.db.commit()
					error_msg = "\n".join(errors)
					frappe.throw(_(error_msg), title=_('Error'))
					...
					error_msg = "[UNEXPECTED ERROR]: {0}".format(ex)
					authnet_log= frappe.get_list("AuthorizeNet Request", fields=["name"], filters={"reference_docname": self.name}, as_list=1)
					authnet_log_user_name = authnet_log[0][0]
					request_authnet_log = frappe.get_doc("AuthorizeNet Request",authnet_log_user_name)
					request_authnet_log.status='Error'
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			data = frappe.db.get_value(self.reference_doctype, self.reference_name, ["company", "customer_name","po_no"], as_dict=1)
		else:
