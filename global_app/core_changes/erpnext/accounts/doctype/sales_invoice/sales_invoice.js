...
frappe.ui.form.on("Sales Invoice", {
	refresh: function (frm) {
		if (cur_frm.doc.items[0].sales_order && cur_frm.is_new()) {
			frappe.call(
				{
					method: "frappe.client.get",
					args: {
						doctype: "Sales Order",
						name: cur_frm.doc.items[0].sales_order,
					},
					callback: function(response) {
						if (!cur_frm.doc.shipment_amount) {
							if (response.message.shipment_cost) {
								frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipment_amount", response.message.shipment_cost);
								cur_frm.refresh_field("shipment_amount");

								frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipping_service", response.message.shipping_service);
								cur_frm.refresh_field("shipping_service");

								frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipment_description", response.message.shipment_details);
								cur_frm.refresh_field("shipment_description")
								frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "reference_so", response.message.name);
								cur_frm.refresh_field("reference_so");
								$.each(response.message.shipment_items || [], function (i, item) {
									var c = cur_frm.add_child("shipment_items");
									c.package = item.package;
									c.tracking = item.tracking;
									c.quantity = item.quantity;
									c.weight = item.weight;
									cur_frm.refresh_field("shipment_items");
								})
								if (response.message.usps_shipment == 1) {
									frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipment_name", "USPS");
									cur_frm.refresh_field("shipment_name");
								}
								else {
									frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipment_name", "UPS");
									cur_frm.refresh_field("shipment_name");
								}

							}
							else {
								frappe.msgprint("No Shipment against this sales order Please process shipment first", "Message");
							}
						}
	...
	validate: function (frm) {
		var name_val = ""
		var ship_amount = frm.doc.shipment_amount
		if (frm.doc.return_against) {
			name_val = "RET - " + frm.doc.return_against
			ship_amount = ship_amount * -1;
		}
		else {
			name_val = frm.doc.po_no ? frm.doc.po_no : "'"
		}
		frappe.call({
			method: "erpnext.accounts.doctype.sales_invoice.sales_invoice.add_shipment_rule",
			args: {
				name: name_val,
				amount: ship_amount,
				total: frm.doc.total
			...
			callback:function(response) {
				if (response.message) {
					frappe.model.set_value(cur_frm.doc.doctype, cur_frm.doc.name, "shipping_rule", response.message.name);
					cur_frm.refresh_field("shipping_rule");
...
erpnext.accounts.SalesInvoiceController = erpnext.selling.SellingController.extend({
	...
	onload: function () {
		...
		cur_frm.refresh_field('apply_discount_on');
		cur_frm.doc.items.forEach(function (item) {
			if (!item.thumbnail && item.item_code) {
				frappe.call(
					{
						method: "frappe.client.get",
						args: {
						...
						callback: function(dl) {
							frappe.model.set_value(item.doctype, item.name, "thumbnail", "<img src=" + dl.message.image + " style='width:40px;height:auto'></img>");
		...
		frappe.call(
			{
				...
				callback: function(dl) {
					frappe.model.set_value(row.doctype, row.name, "thumbnail", "<img src=" + dl.message.image + " style='width:40px;height:auto'></img>");
					cur_frm.refresh_field("thumbnail");
				}
			});
