...
class SalesInvoice(SellingController):
	...
	def send_email(self):
				if self.do_not_send_email_on_submit == 1:
					return
				try:
					from frappe.utils.background_jobs import enqueue
					if self.items[0].sales_order:
						so = frappe.get_doc("Sales Order",self.items[0].sales_order)
						if self.contact_person:
							contact = frappe.get_doc("Contact",self.contact_person)
							recepient=contact.billing_email

						else:
							billing_address=frappe.get_doc("Address",self.customer_address)
							recepient=billing_address.email_id

						if recepient:
							email_args = {
								"recipients": recepient,
								"sender": None,
								"subject": "Processed Order For "+str(so.transaction_date),
								"message": """ Dear <b>{0}</b> ,
												<br>
												We are pleased to let you know your order for the date {1} has been processed and shipped.
												<br>
												We appreciate your business and support. Should you have any questions please give us a call at 951-902-2590 or email us back. 
												<br>
												<br>
												Thank you,
												<br>
												<b>Accounting Department</b><br>
												6251 box springs Blvd<br>
												riverside<br>
												CA 92507<br>
												
												USA
											
								""".format(so.customer,str(so.transaction_date)),
								"now": True,
								"attachments": [frappe.attach_print("Sales Invoice", self.name,
									file_name=self.name, print_format="COBB Sales Invoice")]}
							enqueue(method=frappe.sendmail, queue='short', timeout=300, is_async=True, **email_args)
							frappe.msgprint("Billing email sent successfuly ")
						else:
							frappe.msgprint("Billing email address does not exists")
						return True
					#return "email sent"
				except:
					frappe.log_error(frappe.get_traceback(), 'Sales Invoice '+self.name)
					pass
...
#custom code
@frappe.whitelist()  
def update_grand_total(name,grand_total):
    	# frappe.client.set_value("Sales Invoice", name, "grand_total", grand_total)
    	# return "ok"
		pass

@frappe.whitelist(allow_guest=True)  
def add_shipment_rule(name,amount,total):
	if name:
		try:
				if frappe.db.exists("Shipping Rule","UPS-Ground-"+name):
					doc = frappe.get_doc("Shipping Rule", "UPS-Ground-"+name)
					doc.update({
					...
					frappe.db.commit()
					return doc
				if frappe.db.exists("Shipping Rule","SHPRL-"+name):
					doc = frappe.get_doc("Shipping Rule", "SHPRL-"+name)
					doc.update({
						"shipping_amount":amount
					})
					doc.save(ignore_permissions = True)
				...
				doc = frappe.get_doc({
							...
							"company":"Global Advertising Solution",
							"account":"7540 - Freight-Out - Global",
							"cost_center":"Global - Global",
				...
				frappe.throw("Error creating Shipment Rule")


@frappe.whitelist(allow_guest=True)  
def add_shipment_rule1():
					# SI = frappe.get_all('Sales Invoice',filters={},fields=['*']);
					# count =0
					# for doc in SI:
					# 			try:
													
					# 								if not doc.shipping_rule:
					# 													#count=count+1
					# 													# SHP = frappe.get_doc({
					# 													# 	"doctype":"Shipping Rule",
					# 													# 	"label":"UPS-Ground-"+doc.po_no,
					# 													# 	"shipping_rule_type":"Selling",
					# 													# 	"company":"Global",
					# 													# 	"account":"Sales - Global",
					# 													# 	"cost_center":"Global - Global",
					# 													# 	"calculate_based_on":'Fixed',
					# 													# 	"shipping_amount":doc.shipment_amount	
					# 													# })
					# 													# SHP.save(ignore_permissions = True)
					# 													# #doc.submit()
					# 													# frappe.db.commit()
					# 													do_inv =  frappe.get_doc('Sales Invoice',doc.name) 
					# 													do_inv.update({
					# 														'shipping_rule':"UPS-Ground-"+doc.po_no,
					# 														'total_taxes_and_charges':doc.shipment_amount,
					# 														'taxes':[
					# 															{
					# 																"doctype":"Sales Taxes and Charges",
					# 																"charge_type":"Actual",
					# 																"description":"UPS-Ground-SINV-00303",
					# 																"account_head":"Sales - Global",
					# 																"cost_center":"Global - Global",
					# 																"tax_amount":doc.shipment_amount,
					# 																"total":float(doc.total) + float(doc.shipment_amount)
					# 															}
					# 														], 
					# 														'grand_total':float(doc.total) + float(doc.shipment_amount),
					# 														'rounded_total':float(doc.total) + float(doc.shipment_amount),
					# 														'outstanding_amount':float(doc.total) + float(doc.shipment_amount),
					# 													})
					# 													do_inv.save(ignore_permissions = True)
					# 													do_inv.submit()
					# 													frappe.db.commit()
					# 													count=count+1
					# 			except:
					# 				pass						
					# return count	
					doc =  frappe.get_doc('Sales Invoice',"SINV-00256") 
					doc.update({
						'shipping_rule':"UPS-Ground-"+doc.po_no,
						'total_taxes_and_charges':doc.shipment_amount.split('$')[1],
						'taxes':[
							{
								"doctype":"Sales Taxes and Charges",
								"charge_type":"Actual",
								"description":"UPS-Ground-SINV-00303",
								"account_head":"7540 - Freight-Out - Global",
								"cost_center":"Global - Global",
								"tax_amount":doc.shipment_amount.split('$')[1],
								"total":float(doc.total) + float(doc.shipment_amount.split('$')[1])
							}
						], 
						'grand_total':float(doc.total) + float(doc.shipment_amount.split('$')[1]),
						'rounded_total':float(doc.total) + float(doc.shipment_amount.split('$')[1]),
						'outstanding_amount':float(doc.total) + float(doc.shipment_amount.split('$')[1]),
					})
					doc.save(ignore_permissions = True)
					doc.submit()
					frappe.db.commit()
...
def update_gl_entries():
			...
			count =0
			for doc in SI:
						frappe.db.sql("""UPDATE `tabGL Entry` set credit=%s, credit_in_account_currency=%s  where against_voucher is NULL and voucher_no=%s """ ,(doc.grand_total, doc.grand_total, doc.name))
						frappe.db.commit()
						frappe.db.sql("""UPDATE `tabGL Entry` set debit=%s, debit_in_account_currency=%s  where against_voucher = %s and voucher_no=%s """ ,(doc.grand_total, doc.grand_total, doc.name, doc.name))
						frappe.db.commit()
						count+=1
...
@frappe.whitelist(allow_guest=True) 
def correct_sr():
			SI = frappe.get_all('Sales Invoice',filters={"name":('in',["SINV-00867","SINV-00866","SINV-00784"])},fields=['*'])#,"name":"SINV-00055"
			count =0
			for doc in SI:
						#print(doc.shipment_amount)
						if doc.shipment_amount:
							#print(doc.shipment_amount)
							#if  doc.shipping_rule and doc.shipment_amount > 0 and doc.total_taxes_and_charges == 0:
									# if frappe.db.exists("Shipping Rule","UPS-Ground-"+doc.po_no):
									# 	SHP = frappe.get_doc("Shipping Rule", "UPS-Ground-"+doc.po_no)
									# 	SHP.update({
									# 		"shipping_amount":doc.shipment_amount
									# 	})
									# 	SHP.save(ignore_permissions = True)
									...
									# 	})
									# 	SHP.save(ignore_permissions = True)
									# frappe.db.sql("""UPDATE `tabSales Invoice` set total_taxes_and_charges=%s where name=%s """ ,(doc.shipment_amount,  doc.name))
									# frappe.db.commit()
									# if len(frappe.get_all('Sales Taxes and Charges',filters={"parent":doc.name},fields=['tax_amount','name'])) == 0:
							ST=frappe.get_doc(
								{
									"doctype":"Sales Taxes and Charges",
									"charge_type":"Actual",
									"description":"UPS-Ground-"+doc.name,
									"account_head":"7540 - Freight-Out - Global",
									"cost_center":"Global - Global",
									"tax_amount":doc.shipment_amount,
									"base_tax_amount_after_discount_amount":doc.shipment_amount,
									"total":float(doc.total) + float(doc.shipment_amount),
									"base_total":float(doc.total) + float(doc.shipment_amount),
							...
							frappe.db.commit()
							# print(frappe.get_all('Shipping Rule',filters={"name":"UPS-Ground-"+doc.po_no},fields=['name','shipping_amount']))
							# print(frappe.get_all('Sales Taxes and Charges',filters={"parent":doc.name},fields=['tax_amount','parent']))
							# print(doc.name+" : "+ str(doc.shipment_amount))
							# print(doc.shipping_rule)
							print(count)
							count+=1

@frappe.whitelist()						
def send_batch_email(name):
	try:
		from frappe.utils.background_jobs import enqueue
		import json
		source_name = json.loads(name)
		customer=[]
		for name in source_name:
			...
			if len(customer) != 0:
				...
				for cust in customer:
					if cust['customer_name'] == si.customer:
						cust['inv'].append(name)
						flage=True
				if not flage:
					customer.append({"customer_name":si.customer,"inv":[name]})
			else:
				customer.append({"customer_name":si.customer,"inv":[name]})
		for cst in customer:
			attachments=[]
			for inv in cst['inv']:
				attachments.append(frappe.attach_print("Sales Invoice", inv,
						file_name=inv, print_format="Standard"))
			
			si = frappe.get_doc("Sales Invoice",cst['inv'][0])
			if si.items[0].sales_order:
				so = frappe.get_doc("Sales Order",si.items[0].sales_order)
				contact = frappe.get_doc("Contact",so.contact_person)
				if contact.billing_email:
					email_args = {
						"recipients": "sajid.eycon@gmail.com",#contact.billing_email,
						"sender": None,
						"subject": "Processed Order For "+str(so.transaction_date),
						"message": """ Dear <b>{0}</b> ,
										<br>
										...
										<br>
										<b>Accounting Department</b><br>
										Distrabution Center - East<br>
										3380 Florence Rd Suite#200<br>
										Powder Springs , GA 30127<br>
										951-902-2590<br>
										USA
									
						""".format(cst["customer_name"],str(so.transaction_date)),
						"now": True,
