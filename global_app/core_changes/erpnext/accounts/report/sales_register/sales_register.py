...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	invoice_income_map, invoice_tax_map = get_invoice_tax_map(invoice_list,
		invoice_income_map, income_accounts)
	#msgprint(str(invoice_income_map))
	#Cost Center & Warehouse Map
	invoice_cc_wh_map = get_invoice_cc_wh_map(invoice_list)
	invoice_so_dn_map = get_invoice_so_dn_map(invoice_list)
	company_currency = frappe.get_cached_value('Company',  filters.get("company"),  "default_currency")
	...
	for inv in invoice_list:
			...
			#msgprint(str(income_amount))
			# net total
			#row.append(base_net_total or inv.base_net_total)
			row.append(inv.base_net_total)
			#msgprint(str(inv))
			...
			# total tax, grand total, outstanding amount & rounded total
			row += [total_tax,inv.discount_amount,inv.shipment_amount, inv.base_grand_total, inv.base_rounded_total, inv.outstanding_amount]
			#msgprint(str(row))
			data.append(row)
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = columns + income_columns + [_("Net Total") + ":Currency/currency:120"] + tax_columns + \
		[_("Total Tax") + ":Currency/currency:120",_("Discount Amount") + ":Currency/currency:120",_("Shipment Amount") + ":Currency/currency:120", _("Grand Total") + ":Currency/currency:120",
		_("Rounded Total") + ":Currency/currency:120", _("Outstanding Amount") + ":Currency/currency:120"]
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select name, posting_date, debit_to, project, customer, 
		customer_name, owner, remarks, territory, tax_id, customer_group,
		base_net_total,discount_amount,shipment_amount, base_grand_total, base_rounded_total, outstanding_amount {0}
		from `tabSales Invoice`
		where docstatus = 1 %s order by posting_date desc, name desc""".format(additional_query_columns or '') %
		conditions, filters, as_dict=1)
