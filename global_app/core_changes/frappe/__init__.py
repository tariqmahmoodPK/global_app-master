...
def get_site_config(sites_path=None, site_path=None):
	...
	if site_path:
		...
		if os.path.exists(site_config):
			...
			try:
				sys.stdout.write("{0} does not exist".format(local.site))
				sys.exit(1)
			except:
				sys.exit(1)
...
def msgprint(msg, title=None, raise_exception=0, as_table=False, indicator=None, alert=False):
	...
	if flags.print_messages and out.msg:
		try:
			sys.stdout.write("Message: " + repr(out.msg).encode("utf-8"))
		except:
			pass
		...
		for app in [app_name] if app_name else get_installed_apps(sort=True):
			...
			try:
				...
				if local.flags.in_install_app:
					...
					pass
				try:
					sys.stdout.write('Could not find app "{0}"'.format(app_name))
				except:
					pass
				if not request:
					sys.exit(1)
